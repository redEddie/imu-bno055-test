...
### 6단계: 나침반(heading) — 실용은 NDOF 가 정답 (`stage6_compass.py`)
세 heading 비교: `RAW(.)` 틸트보상 없음 · `TILT(*)` 쿼터니언 틸트보상 ·
`NDOF(#)` BNO055 9축 절대 yaw(정답).

**결론**: 로봇 heading 은 **BNO055 의 NDOF yaw(#)를 그대로 쓰는 게 정답**이다.
칩이 가속도+자이로+자력계를 융합하고 하드/소프트아이언 보정까지 내부에서 하므로
가장 정확하다. heading 제어기도 이 값을 기준 삼으면 된다.

자력계로 직접 heading 을 구하려면 (1) mag 축이 가속도 축과 1:1 로 안 맞고
(2) mag 전용 캘리브레이션이 따로 필요해 손이 많이 간다. 다만 **쿼터니언으로
mag 벡터를 월드 좌표로 회전**(`geometry.quat_tilt_heading`)하면 축 정렬 없이
틸트보상이 되어, 기울여도 TILT(*)가 NDOF 를 **~1.5°** 로 추종한다(상수 offset 은
자북 기준 차이). "기울여도 방위가 맞는 나침반"의 원리는 이렇게 확인했고,
정밀·실용 heading 은 NDOF 를 쓴다.

> ⚠ 자력계 보정 필수: `cal` 마지막 자리(mag)가 3 이어야 NDOF yaw 가 정확하다.
> `calib_offsets.json` 이 있으면 stage6 가 시작 시 자동 복원하고, 없으면 8자로
> 휘저어 보정하라(`calibrate_and_save.py`).
